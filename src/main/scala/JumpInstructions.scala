  ...
  val JALR =
    new RTypeInstruction {
      def perform( cpu: CPU, rs: Int, rt: Int, rd: Int, shamt: Int, func: Int ) = {
        cpu.put( rd, cpu.pc + 8 )
        cpu.delay1( rs, (cpu, rsv) => if ((rsv & 0x3) != 0) cpu.exception(ADDRESS_Exception) else cpu.pc = rsv )
      }
    }
  val JR =
    new RTypeInstruction {
      def perform( cpu: CPU, rs: Int, rt: Int, rd: Int, shamt: Int, func: Int ) =
        cpu.delay1( rs, (cpu, dest) => if ((dest & 0x3) != 0) cpu.exception(ADDRESS_Exception) else cpu.pc = dest )
    }
